...
export default async function(ctx) {
  ...
  const colors = {
    ...
    chong: { light: '#7B1FA2', dark: '#CE93D8' },
    countdown: {
      legal: '#FF6B6B',
      term: '#4ECDC4',
      folk: '#FFD93D',
  ...
  const chongAnimal = animals[(dZhi + 6) % 12];
  const chongGanZhi = stems[cIndex % 10] + branches[cIndex % 12];
  ...
  const dayGZ = gzAPI ? gzAPI.day : (dayGanZhi + "日");
