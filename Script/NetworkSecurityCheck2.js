/* 来自群友@BP999X @Aston无私分享，AI修改。
 * Egern widget: Network Security Check (simplified, no rule stats)
 *
 ...
 *   SHOW_IP=false
 *   REFRESH_MINUTES=10
 *   FILTER_ENABLED=true   (set to "false" to show filter as disabled)
 */
...
/**
 * Filter check: simply reports whether filtering is enabled.
 * Controlled by env FILTER_ENABLED (default true).
 */
function filterCheck(ctx) {
  const enabled = boolEnv(ctx, 'FILTER_ENABLED', true);
  if (enabled) {
    return check('filter', '规则过滤', '已启用', 'pass', '过滤规则正在保护您的网络');
  } else {
    return check('filter', '规则过滤', '已停用', 'warn', '规则过滤未开启，请检查配置');
  }
}
